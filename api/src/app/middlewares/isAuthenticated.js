...
const isAuthenticated = async (req, res, next) => {
    try {
        ...
        let user = await AdminUsersService.findUserById(userId);
        if (user) {
            req.userId = userId;
            req.userType = "Admin";
            return next();
        ...
        user = await ClientUsersService.findUserById(userId);
        if (user) {
            req.userId = userId;
            req.userType = "Client";
            return next();
        ...
        throw new AppError(403, "Acesso não autorizado. Usuário não encontrado.");
    } catch (error) {
